...
>
	<!-- Card -->
	<div
		class="relative w-40 h-60 rounded-xl shadow-2xl bg-white border-4 {isRed
			? 'text-red-600 border-red-200'
			: 'text-slate-900 border-slate-300'}"
	>
		<div class="absolute top-2 left-2 leading-tight">
			<div class="font-bold text-2xl">{royal.rank}</div>
			<div class="text-xl">{SUIT_GLYPH[royal.suit]}</div>
		</div>
		<div class="absolute inset-0 flex flex-col items-center justify-center gap-1">
			<div class="text-7xl">{SUIT_GLYPH[royal.suit]}</div>
			<div class="text-sm uppercase tracking-widest font-semibold">{rankLabel}</div>
		</div>
